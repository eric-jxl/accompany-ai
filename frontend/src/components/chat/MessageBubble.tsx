...
const EmotionDisplay: React.FC<EmotionDisplayProps> = ({emotionAnalysis}) => {
    ...
    return (
        <div
            className="mt-2 p-2 bg-purple-50 rounded-lg border border-purple-200 dark:bg-purple-900/30 dark:border-purple-700">
...
const DebugInfo: React.FC<DebugInfoProps> = ({debugInfo}) => {
    ...
    return (
        <div
            className="mt-2 border border-gray-200 rounded-lg overflow-hidden bg-gray-50 dark:border-gray-700 dark:bg-gray-800">
            <button
          ...
          <pre
              className="text-xs text-gray-600 whitespace-pre-wrap font-mono leading-relaxed overflow-x-auto dark:text-gray-400">
            {debugInfo}
          </pre>
                </div>
            )}
    ...
    if (isSystem) {
        return (
            <div className="flex justify-center my-4">
                <div
                    className="px-4 py-2 bg-gray-100 text-gray-700 text-sm rounded-full border border-gray-200 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">
                    {message.content}
                </div>
            </div>
        );
            ...
            <div className={`max-w-md lg:max-w-xl xl:max-w-2xl ${isUser ? 'order-2' : 'order-1'}`}>
                ...
                {!isUser && message.mode && debugMode && (
                    <div className="mb-2 flex items-center space-x-2">
                        {/* 模式指示器 */}
                        <div
                            className="inline-flex items-center space-x-1 text-xs px-2 py-1 rounded-full bg-gradient-to-r from-purple-100 to-indigo-100 text-purple-700 dark:from-purple-900/30 dark:to-indigo-900/30 dark:text-purple-300">
